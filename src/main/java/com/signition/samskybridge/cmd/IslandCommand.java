...
void sendHelp(Player p, int page){
        ...
        p.sendMessage(Text.color("&7/섬 관리 강등 <닉> &f: 부섬장 → 섬원"));
            p.sendMessage(Text.color("&7/섬 초대 <닉> &f: 팀에 초대 (/is team invite)"));
            ...
            if (p.hasPermission("samsky.admin")){
                ...
                p.sendMessage(Text.color("&7/섬 설정 (블럭경험치 제거됨) <블럭> <xp> &f: blocks.yml 수정"));
...
case "설정": {
                ...
                // /섬 설정 (블럭경험치 제거됨) <블럭> <xp>
                p.sendMessage(com.signition.samskybridge.util.Text.color("&7블럭경험치 기능은 제거되었습니다.")); return true;}
                // /섬 설정 저장
                if ("저장".equals(args[1])){
                    plugin.saveConfig();
